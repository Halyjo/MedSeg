...
def preprocess2d(mode='train'):
    ...
    ## Processing
    start = time()
    for f in tqdm(os.listdir(config[f"{mode}_volumes_path"]), desc=f"{mode} 2d processing"):
        ## Get Volume matrices
        ct = sitk.ReadImage(os.path.join(config[f"{mode}_volumes_path"], f), sitk.sitkInt16)
        ct_array = sitk.GetArrayFromImage(ct)

        seg = sitk.ReadImage(os.path.join(config[f"{mode}_labels_path"], f.replace('volume', 'segmentation')), sitk.sitkUInt8)
...
def preprocess3d(mode='train'):
    ...
    dirpaths = [
        config[f"dst_{mode}_labels_liver_path"],
        config[f"dst_{mode}_labels_lesion_path"],
        config[f"dst_{mode}_volumes_path"],
    ]
    for p in dirpaths:
        if not os.path.exists(p):
            os.mkdir(p)
    ...
    ## Processing
    start = time()
    for f in tqdm(os.listdir(config[f"{mode}_volumes_path"]), desc=f"{mode} processing"):
        ## Get Volume matrices
        ...
        sitk.WriteImage(new_ct, os.path.join(config[f"dst_{mode}_volumes_path"], f))
        sitk.WriteImage(liver_sitk_lab, os.path.join(config[f"dst_{mode}_labels_liver_path"], f.replace('volume', 'segmentation').replace('.nii', '.nii.gz')))
        sitk.WriteImage(lesion_sitk_lab, os.path.join(config[f"dst_{mode}_labels_lesion_path"], f.replace('volume', 'segmentation').replace('.nii', '.nii.gz')))
    print("Finished {} preprocessing in: {:02}s".format(mode, time() - start))
